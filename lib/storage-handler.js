 ...
 *
 * @param provider
 * @param providerFolderKey
 * @returns {boolean}
 */
exports.providerHasFolderSupport = function(provider, providerFolderKey) {
  return 'string' === typeof providerFolderKey ||
      provider === 'amazon' ||
      provider === 'filesystem';
};
 ...
 * @param cb
 */
exports.isValidFolder = function(folder, cb) {
  if (!folder || '/(^)\.\.($)/'.test(folder)) {
    cb && process.nextTick(cb.bind(null, new Error(g.f('Invalid name: %s', folder))));
    if (!cb) {
      console.error(g.f('{{FileSystemProvider}}: Invalid name: %s', folder));
    }
    return false;
  }
  return true;
 ...
 * @param providerFolderKey
 */
exports.setProviderFolder = function(options, folder, provider, providerFolderKey) {
  if (!exports.providerHasFolderSupport(provider, providerFolderKey)) {
    return;
  }
  options = options || {};
  switch (provider) {
    ...
    case 'amazon':
      ...
      break;
    default:
      if ('string' === typeof providerFolderKey) {
        options[providerFolderKey] = folder;
      }
      break;
  }
};
...
function(provider, providerName, container, file, folder, options) {
  if (folder && !exports.isValidFolder(folder)) return;
  if (folder) {
    exports.setProviderFolder(options, folder, providerName);
  }
 ...
 */
exports.getFiles = function(provider, providerName, container, folder, options, cb) {
  if (typeof options === 'function' && !cb) {
        // options argument is not present
    cb = options;
    options = {};
  }
  if (folder && !exports.isValidFolder(folder, cb)) return;
  if (folder && exports.providerHasFolderSupport(providerName)) {
    exports.setProviderFolder(options, folder.replace(/^\/+/g, ''), providerName);
  }
  return provider.getFiles(container, options, function(err, files) {
    if (err) {
      cb(err, files);
    } else {
 ...
 * @returns {*|void|undefined}
 */
exports.getFile = function(provider, providerName, container, file, folder, cb) {
  if (folder && !exports.isValidFolder(folder, cb)) return;
  if (folder && providerName === 'filesystem') {
    provider.folder = folder;
  } else if (folder && exports.providerHasFolderSupport(providerName)) {
    file = path.join(folder, file);
  }
  return provider.getFile(container, file, function(err, f) {
    return cb(err, map(f));
  });
};
 ...
 * @returns {*|void|undefined}
 */
exports.removeFile = function(provider, providerName, container, file, folder, cb) {
  if (folder && !exports.isValidFolder(folder, cb)) return;
  if (folder && providerName === 'filesystem') {
    provider.folder = folder;
  } else if (folder && exports.providerHasFolderSupport(providerName)) {
    file = path.join(folder.replace(/^\/+/g, ''), file);
  }
  return provider.removeFile(container, file, cb);
};
...
exports.upload = function(provider, req, res, options, cb) {
  ...
  var folder = options.folder || req.query.folder || '';
  var providerName = options.provider;
  var providerFolderKey = options.providerFolderKey;
  var fields = {};
  var files = {};
  form.handlePart = function(part) {
    ...
    if (folder && providerName === 'filesystem') {
      ...
      exports.providerHasFolderSupport(providerName, providerFolderKey)
    ) {
      exports.setProviderFolder(uploadParams, folder.replace(/^\/+/g, ''), providerName, providerFolderKey);
    }
 ...
 * @param {Object} provider The storage service provider
 * @param {String} providerName The storage service provider name
 * @param {Request} req The HTTP request
 * @param {Response} res The HTTP response
 ...
 */
exports.download =
function(provider, providerName, req, res, container, file, folder, cb) {
  ...
  if (folder && providerName === 'filesystem') {
    ...
    params.remote = path.join(folder.replace(/^\/+/g, ''), params.remote);
  }
