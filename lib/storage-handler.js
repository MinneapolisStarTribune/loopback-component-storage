 ...
 * @returns {*}
 */
exports.trimSlashes = function(folder) {
  if (typeof 'string' !== folder) {
    return folder;
  }
 ...
 * @returns {*}
 */
exports.setFolder = function(folder, options, file) {
  this.trimSlashes(folder);
  if (folder && typeof 'object' === options) {
    options.folder = folder;
  }
  return folder;
};

exports.getFolder = function(container, folder, options) {
  return container;
};
...
exports.isValidFolder = function(folder, cb) {
  var attemptDirChange = '/(^)\.\.($)/';
  if (!folder || attemptDirChange.test(folder)) {
    cb && process.nextTick(cb.bind(null, new Error(g.f('Invalid path: %s', folder))));
    if (!cb) {
      console.error(g.f('{{FileSystemProvider}}: Invalid path: %s', folder));
    }
    return false;
  }
  return true;
 ...
 * @param options
 */
exports.uploadStream = function(provider, container, file, folder, options) {
  if (folder && !this.isValidFolder(folder)) return;
  if (folder) {
    this.setFolder(folder, options, file);
  }
 ...
 * @param options
 */
exports.downloadStream = function(provider, container, file, folder, options) {
  if (folder && !this.isValidFolder(folder)) return;
  if (folder) {
    this.setFolder(folder, options, file);
  }
 ...
 */
exports.getFiles = function(provider, container, folder, options, cb) {
  if (typeof options === 'function' && !cb) {
        // options argument is not present
    cb = options;
    options = {};
  }
  if (folder && !this.isValidFolder(folder, cb)) return;
  if (folder) {
    this.setFolder(folder, options);
  }
 ...
 * @returns {*|void|undefined}
 */
exports.getFile = function(provider, container, file, folder, cb) {
  if (folder && !this.isValidFolder(folder, cb)) return;
  if (folder) {
    this.setFolder(folder, provider, file);
  }
 ...
 * @returns {*|void|undefined}
 */
exports.removeFile = function(provider, container, file, folder, cb) {
  if (folder && !this.isValidFolder(folder, cb)) return;
  if (folder) {
    this.setFolder(folder, provider, file);
  }
...
exports.upload = function(provider, req, res, options, cb) {
  ...
  form.handlePart = function(part) {
    ...
    if (folder) {
      this.setFolder(folder, uploadParams, file.name);
    }
 ...
 */
exports.download = function(provider, req, res, container, file, folder, cb) {
  ...
  if (folder && !this.isValidFolder(folder, cb)) return;
  if (folder) {
    this.setFolder(folder, params, params.file);
  }
