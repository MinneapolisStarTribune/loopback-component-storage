...
FileSystemProvider.prototype.validateFileName =
FileSystemProvider.prototype.validateContainerName = function(name, cb) {
...
FileSystemProvider.prototype.createContainer = function(options, cb) {
  ...
  this.validateContainerName(name, cb) && fs.mkdir(dir, options, function(err) {
    if (err) {
      return cb && cb(err);
    }
...
FileSystemProvider.prototype.destroyContainer = function(containerName, cb) {
  if (!this.validateContainerName(containerName, cb)) return;
...
FileSystemProvider.prototype.upload = function(options, cb) {
  ...
  var folder = options.folder || this.folder;
  var folderPath = path.join(this.root, container);
  if (folder) {
...
FileSystemProvider.prototype.download = function(options, cb) {
  var container = options.container;
  if (!this.validateContainerName(container, cb)) {
  ...
  var file = options.remote;
  if (!this.validateFileName(file, cb)) {
    return readStreamError(
      new Error(g.f('{{FileSystemProvider}}: Invalid name: %s', file)),
      cb
    );
...
FileSystemProvider.prototype.getFiles = function(container, options, cb) {
  ...
  var self = this;
  if (!this.validateContainerName(container, cb)) return;

  if (options.folder) {
    container = path.join(container, options.folder);
  }
...
FileSystemProvider.prototype.getFile = function(container, file, cb) {
  var self = this;
  if (!this.validateContainerName(container, cb)) return;
  if (!this.validateFileName(file, cb)) return;

  var root = path.join(this.root, container);
  if (this.folder) {
    root = path.join(root, this.folder);
  }
...
FileSystemProvider.prototype.getUrl = function(options) {
  options = options || {};
  return path.join(this.root, options.container, options.path);
};
